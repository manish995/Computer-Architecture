...
uint8_t simulation_pause[NUM_CPUS] = {};
uint64_t warmup_instructions = 0;
uint64_t simulation_instr_bw_resume_pause = 0;
uint64_t others = 0;
std::array<CACHE*, NUM_CACHES> save_cache_stat;
...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i<NUM_CACHES; i++){
     save_cache_stat[i] = (CACHE*)(malloc(sizeof(CACHE)));
  }
  ...
  static struct option long_options[] = {{"hide_heartbeat", no_argument, 0, 'h'},
  ...
  while (std::any_of(std::begin(simulation_complete), std::end(simulation_complete), std::logical_not<uint8_t>())) {
    ...
    for (std::size_t i = 0; i < ooo_cpu.size(); ++i) {
      ...
      while (ooo_cpu[i]->fetch_stall == 0 && ooo_cpu[i]->instrs_to_read_this_cycle > 0) {
        ooo_model_instr instr = traces[i]->get();

        if(instr.extra_bit == 0){
          warmup_instructions++;
        }
        else if(instr.extra_bit == 1){
          simulation_instr_bw_resume_pause++;
          if(warmup_complete[i] == 0){
            warmup_complete[i] = 1;
            all_warmup_complete++;
          }
          if(simulation_pause[i] == 1){
            simulation_pause[i] = 0;
            
            //Simulation is resumed, copy back the stats 
            for(int i = 0; i<NUM_CPUS; i++){
              for(int j = 0; j<NUM_TYPES; j++){
                
                caches[i]->sim_access[i][j] = save_cache_stat[i]->sim_access[i][j];
                caches[i]->sim_hit[i][j] = save_cache_stat[i]->sim_hit[i][j];
                caches[i]->sim_miss[i][j] = save_cache_stat[i]->sim_miss[i][j];
              }
              caches[i]->pf_requested = save_cache_stat[i]->pf_requested;
              caches[i]->pf_issued = save_cache_stat[i]->pf_issued;
              caches[i]->pf_useful = save_cache_stat[i]->pf_useful;
              caches[i]->pf_useless = save_cache_stat[i]->pf_useless;
              caches[i]->pf_fill = save_cache_stat[i]->pf_fill;

              caches[i]->total_miss_latency = save_cache_stat[i]->total_miss_latency;

              caches[i]->RQ_ACCESS = save_cache_stat[i]->RQ_ACCESS;
              caches[i]->RQ_MERGED = save_cache_stat[i]->RQ_MERGED;
              caches[i]->RQ_TO_CACHE = save_cache_stat[i]->RQ_TO_CACHE;

              caches[i]->WQ_ACCESS = save_cache_stat[i]->WQ_ACCESS;
              caches[i]->WQ_MERGED = save_cache_stat[i]->WQ_MERGED;
              caches[i]->WQ_TO_CACHE = save_cache_stat[i]->WQ_TO_CACHE;
              caches[i]->WQ_FORWARD = save_cache_stat[i]->WQ_FORWARD;
              caches[i]->WQ_FULL = save_cache_stat[i]->WQ_FULL;

            }
          }
        }
        else if((instr.extra_bit == 2 )&& (simulation_pause[i] == 0)){
          simulation_pause[i] = 1;
          others++;
          //simulation is paused, save the stats
          for(int i = 0; i<NUM_CPUS; i++){
            for(int j = 0; j<NUM_TYPES; j++){
              
              save_cache_stat[i]->sim_access[i][j] = caches[i]->sim_access[i][j];
              save_cache_stat[i]->sim_hit[i][j] = caches[i]->sim_hit[i][j];
              save_cache_stat[i]->sim_miss[i][j] = caches[i]->sim_miss[i][j];
            }
            save_cache_stat[i]->pf_requested = caches[i]->pf_requested;
            save_cache_stat[i]->pf_issued =   caches[i]->pf_issued;
            save_cache_stat[i]->pf_useful =   caches[i]->pf_useful;
            save_cache_stat[i]->pf_useless = caches[i]->pf_useless;
            save_cache_stat[i]->pf_fill = caches[i]->pf_fill;

            save_cache_stat[i]->total_miss_latency = caches[i]->total_miss_latency;

            save_cache_stat[i]->RQ_ACCESS = caches[i]->RQ_ACCESS;
            save_cache_stat[i]->RQ_MERGED = caches[i]->RQ_MERGED;
            save_cache_stat[i]->RQ_TO_CACHE = caches[i]->RQ_TO_CACHE;

            save_cache_stat[i]->WQ_ACCESS = caches[i]->WQ_ACCESS;
            save_cache_stat[i]->WQ_MERGED = caches[i]->WQ_MERGED;
            save_cache_stat[i]->WQ_TO_CACHE = caches[i]->WQ_TO_CACHE; 
            save_cache_stat[i]->WQ_FORWARD = caches[i]->WQ_FORWARD;
            save_cache_stat[i]->WQ_FULL = caches[i]->WQ_FULL;



          }
        }
        else if((instr.extra_bit == -1) && (simulation_complete[i] == 0)){
          check = 1;
        ...
        ooo_cpu[i]->init_instruction(instr);
      }
      ...
      // simulation complete
      if ((all_warmup_complete > NUM_CPUS) && (check == 1)){
        check = 0;
  ...
  cout<<"simulation instructions: "<<simulation_instr_bw_resume_pause<<endl;
  cout<<"Others instructions : "<<others<<endl;
