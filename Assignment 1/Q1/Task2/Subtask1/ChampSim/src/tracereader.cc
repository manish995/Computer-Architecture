...
void tracereader::open(std::string trace_string)
{
  ...
  char arr1[32];
  char arr2[32];
  fread(arr1, 32, 1, trace_file);
  fread(arr2, 32, 1, trace_file);
  std::cout<<"Warmup_instructions: "<<construct_int(arr1)<<'\n';
  std::cout<<"Simulation_instructions: "<<construct_int(arr2)<<'\n';
  Total_warmup=construct_int(arr1);
  Total_simulation=construct_int(arr2);
