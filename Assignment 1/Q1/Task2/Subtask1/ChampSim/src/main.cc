...
int main(int argc, char** argv)
{
  ...
  for (int i = optind; i < argc; i++) {
    ...
    tracereader * newtrace=get_tracereader(argv[i], traces.size(), knob_cloudsuite);
    traces.push_back(newtrace);
    warmup_instructions = newtrace->Total_warmup;
    simulation_instructions = newtrace->Total_simulation;
